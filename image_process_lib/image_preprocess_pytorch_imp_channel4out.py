...
class image_processing:
    ...
    def output_image_array(self, image_tensor, processed_image):
        """Extract and amplify pixel values based on max channel index and its mean-variance difference (z-score)."""
        ...
        # Gather pixel values from original image based on max channel index: [B, 1, H, W]
        selected_pixel = torch.gather(image_tensor, dim=1, index=max_channel_index)
    
        # Gather the corresponding processed value (z-score = (pixel - mean) / std) from the max channel: [B, 1, H, W]
        selected_z = torch.gather(processed_image, dim=1, index=max_channel_index)
    
        # Amplify based on the mean-variance difference (z-score):
        # - Use clamp to consider only positive deviations (pixel > local mean) for amplification to enhance standout features.
        # - Amplification factor = 2 + max(0, z), so higher positive z (larger deviation relative to local std) leads to greater amplification.
        # - This pushes pixels further from the local mean in high-deviation areas, enhancing contrast without needing explicit mean/std tensors.
        amplification_factor = 2 + torch.clamp(selected_z, min=0.0)
    
        # Apply amplification to the selected pixel values
        output = selected_pixel * amplification_factor
    
        # Optional: Clip to [0, 255] to maintain original value range (assuming input_tensor is in [0, 255]).
        # Remove or adjust if your pipeline normalizes inputs differently (e.g., to [0, 1]).
        output = torch.clamp(output, min=0.0, max=255.0)
    
        return output  # [B, 1, H, W]

    def extract_image_features(self, input_tensor, kernel=1):
        """Main method to process batched or single images, returning RGB + processed channel."""
        ...
        # input_tensor: [B, 3, H, W], processed_channel: [B, 1, H, W]
