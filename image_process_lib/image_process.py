...
class image_processing:
    ...
    def extract_image_features(self, input_tensor):
        ...
        if isinstance(input_tensor, torch.Tensor):
            ...
            for i in range(batch_size):
                # Get single image (handle both batched and single image)
                if len(input_tensor.shape) == 4:  # Batched: (B, C, H, W)
                    img = input_tensor[i].permute(1, 2, 0)  # Convert from CHW to HWC
                else:  # Single image: (C, H, W)
                    img = input_tensor.permute(1, 2, 0)  # Convert from CHW to HWC
                
                # Convert to numpy for processing
                numpy_image_array = img.detach().cpu().numpy()
                ...
                final_tensor = torch.from_numpy(final_image).float()
                if len(final_tensor.shape) == 2:  # Add channel dimension
                    final_tensor = final_tensor.unsqueeze(0)  # Add channel dimension
                
                processed_images.append(final_tensor)
            
            # Stack all processed images back into batch
            if len(input_tensor.shape) == 4:  # Return batched result
                result = torch.stack(processed_images, dim=0)
            else:  # Return single image result
                result = processed_images[0]
            
            # Move back to the same device as input
            return result.to(input_tensor.device)
        else:
            # Handle numpy array input (legacy)
            processed_channel_images = self.get_processed_image(input_tensor) 
            final_image = self.output_image_array(input_tensor, processed_channel_images)
            return final_image